# EV Technology Quiz

🚗 **Live Quiz**: [Click Here](https://manoj1680.github.io/EV-QUIZ/)

Test your knowledge with this 60-question multiple-choice quiz on Electric Vehicle (EV) technology! Topics include power electronics, battery management, thermal systems, charging infrastructure, and powertrain components.

## Features
✅ 60 questions across 7 EV topics  
✅ Mobile-friendly design  
✅ Instant scoring with percentage  
✅ Answer explanations after quiz  
✅ No login required  
✅ Works on all devices  

## Topics Covered
1. Power Electronics & Thermal Management
2. Battery Systems & Relays  
3. Alternator & Electrical Generation
4. Starting System & Ignition
5. EV Powertrain & Charging
6. Power Flow & Control Units
7. Advanced Topics & Integration

## How to Use
1. Open the link on any device (phone, tablet, computer)
2. Answer all 60 questions (select one option each)
3. Click "Submit Quiz" when finished
4. View your score and review answers

## Share with Friends
Share this link: `https://manoj1680.github.io/ev-quiz/`

---

**Created for educational purposes** - Perfect for automotive students, EV technicians, and engineering enthusiasts.
